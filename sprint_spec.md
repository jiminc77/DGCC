...
- **AMD-5 (unconditional scheduling deferral) (2026-07-25, owner steer 2026-07-25T12:39Z — 집행 커밋 596c860; 등록 게시: DGCC#22 comment 5078537467 및 rd#36 재핀 코멘트[판정자 측, 2026-07-25T12:48Z], 양건 2026-07-25T12:48Z)**: 아래 본문은 DGCC#22 comment 5078537467 게시본 verbatim이다. 핵심 조항: (a) 최종 paired n=7 불변 — s6/s7은 취소가 아닌 연기 (b) 2026-08-18 23:59 KST 기한 내 결과 무관 무조건 실행·GNG-2는 그 전 종결 금지 (c) n=5 구간 결과는 interim·descriptive only — Level-4/C1/C2/C3/mechanism-fallback 주장 불가.

  ### AMD-5 — Unconditional scheduling deferral of V1 seeds 6 and 7, without a change to the final paired sample

  Date and time of registration: **2026-07-25, 21:50 KST**

  **Disclosure at registration.** V1 seed-level held-out outcomes available at this time: seed 0 = 21.5% success / +1.53 mean return; seed 1 = 12.0% / +0.96; seed 2 = 33.0% / +2.24. V1 seed 3 is **training in progress** (no validation-final, checkpoint-selection, or held-out outcome exists). V1 seed 4 has **not started**. All BB outcomes for seeds 0–4, 6, and 7 are known (among them seed 6 = 2.5%, seed 7 = 10.0%). V1 seeds 6 and 7 have **not been launched**, and no V1 outcome, checkpoint-selection result, or held-out result exists for either seed. This amendment is therefore **not outcome-blind** with respect to the partially completed Level-4 campaign; it is registered before any observation of the deferred runs themselves.

  **The final paired seed set remains {0, 1, 2, 3, 4, 6, 7}, paired n = 7** (AMD-3 unchanged: seed 5 excluded as a complete pair, no replacement permitted).

  V1 seeds 6 and 7 are deferred **solely to provide the prospectively registered V2-dev exploratory track with earlier access to the dedicated GPU**. They are not cancelled and will be completed irrespective of V2 performance, mechanism results, the GNG-2 branch, or the intended paper headline.

  Both runs, their locked validation checkpoint selections, one-shot held-out evaluations, and required C1–C3 artifacts must be completed **no later than 2026-08-18, 23:59 KST. GNG-2 may not close before this condition is satisfied.**

  Any five-seed result computed before completion of seeds 6 and 7 is **interim and descriptive only**. It may not support a confirmatory Level-4, C1, C2, C3, or mechanism-fallback claim.

  No replacement seed is permitted. Existing technical-failure and attempt-accounting rules remain in force.

  ---

  한국어 요약: V1 s6·s7을 지금 연기하되(사유 = V2-dev 트랙에 GPU 조기 배정, 일정 결정·성능 무관 — 본 등록 시점에 두 seed의 V1 결과는 존재하지 않음), **2026-08-18 23:59 KST까지 GNG 분기·성능·mechanism 결과와 무관하게 무조건 실행**하여 최종 paired n=7을 유지한다. 그 전의 n=5 중간 결과는 descriptive 전용이며 어떤 confirmatory 주장도 지지할 수 없다. 조건부 복원(결과 의존 실행)은 적응적 표본완성이므로 채택하지 않았다. BB s6/s7 성적이 기지라는 사실을 공시하며, "제외 방향이 보수적"이라는 논거는 보조 설명으로만 사용한다(미관측 paired delta의 방향은 확정 불가).
