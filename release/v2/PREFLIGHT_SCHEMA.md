...
- Owner-pinned isolated repo root `/home/simx2204/v2_research/impl/DGCC`, sparse training sandbox root `/home/simx2204/v2_research/runtime/DGCC-v2-12befdac`, runtime source commit `12befdac…`, and evidence-base commit `8c63ed57…`. These identities are constants, not caller-selected expectations.
...
The environment is pinned beside the code manifest in `v2_runtime_environment.json` and allowlisted per cell as `runtime_environment`. The code manifest pins source bytes only; identical source under a different accelerator torch build or a different simulator revision is a different experiment, and the isolated worktree silently had neither until this was pinned.
