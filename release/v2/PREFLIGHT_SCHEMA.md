...
- Owner-pinned isolated repo root `/home/simx2204/v2_research/impl/DGCC`, sparse training sandbox root `/home/simx2204/v2_research/runtime/DGCC-v2-12befdac`, runtime source commit `12befdac…`, and evidence-base commit `bf086fd7…`. These identities are constants, not caller-selected expectations.
...
- `cells/<...>/launch_dryrun_receipt.json`: the real launcher driven on production argv to `AttemptRegistry.recover`, the first side effect past the asset gate. Records every resolved role, path, and digest with no attempt record, agent, GPU context, or training step.
